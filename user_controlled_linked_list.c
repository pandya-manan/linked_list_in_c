...
struct Node* deleteAtGivenIndex(struct Node* head,int index)
{
    ...
    {
        printf("Deletion is not possible as the length of the list is: %d",lengthOfLinkedList);
        return head;
    }
    else
    {
...
struct Node* deleteAtEnd(struct Node* head)
{
    if(head==NULL)
    {
        printf("The linked list is empty.Hence it is not possible to delete from the last end\n");
        return head;
    }
    struct Node* p=head;
